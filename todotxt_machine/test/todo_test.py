...
def todos():
    return todo.Todos([
        ...
        "Unpack the guest bedroom +Unpacking due:20/10/2013",
        "19/10/2013 Post signs around the neighborhood +GarageSale",
        "x 01/10/2013 @GroceryStore Eskimo pies" ], './todo.txt', './archive.txt')
...
def test_todos_parse_entries(todos):
    ...
    assert todo.raw      == "Unpack the guest bedroom +Unpacking due:20/10/2013"
    assert todo.contexts == []
    assert todo.projects == ["+Unpacking"]
    assert todo.due_date == "20/10/2013"

    todo = todos.todo_items[3]
    assert todo.raw           == "19/10/2013 Post signs around the neighborhood +GarageSale"
    assert todo.contexts      == []
    assert todo.projects      == ["+GarageSale"]
    assert todo.creation_date == "19/10/2013"

    todo = todos.todo_items[4]
    assert todo.raw            == "x 01/10/2013 @GroceryStore Eskimo pies"
    assert todo.contexts       == ["@GroceryStore"]
    assert todo.projects       == []
    assert todo.completed_date == "01/10/2013"
...
def test_context_project_regex(todos):
    todos.update([
        "(A) 24/12/1999 Thank Mom for the dinner @phone @email mom@email.com",
        "(B) Schedule Goodwill pickup +GarageSale NotA+Project @phone",
        "Unpack the guest bedroom +Unpacking due:20/10/2013",
        "19/10/2013 Post signs around the neighborhood +GarageSale",
        "x 01/10/2013 @GroceryStore Eskimo pies" ])
    todo = todos[0]
    assert todo.raw      == "(A) 24/12/1999 Thank Mom for the dinner @phone @email mom@email.com"
...
def test_todos_completed_date(todos):
    assert todos.completed_date("02/03/2011 Document +TodoTxt task format")                  == ""
    assert todos.completed_date("(A) 02/03/2011 Document +TodoTxt task format")              == ""
    assert todos.completed_date("x 03/03/2012 2011-03-02 Document +TodoTxt task format")     == "2012-03-03"
    assert todos.completed_date("x 03/03/2012 (A) 2011-03-02 Document +TodoTxt task format") == "2012-03-03"



def test_todos_creation_date(todos):
    assert todos.creation_date("02/03/2011 Document +TodoTxt task format")                  == "2011-03-02"
    assert todos.creation_date("(A) 02/03/2011 Document +TodoTxt task format")              == "2011-03-02"
    assert todos.creation_date("x 03/03/2012 2011-03-02 Document +TodoTxt task format")     == "2011-03-02"
    assert todos.creation_date("x 03/03/2012 (A) 2011-03-02 Document +TodoTxt task format") == "2011-03-02"


def test_todos_due_date(todos):
    assert todos.due_date("02/03/2011 Document +TodoTxt task format due:2013-10-25") == "2013-10-25"
    assert todos.due_date("02/03/2011 due:2013-10-25 Document +TodoTxt task format") == "2013-10-25"
    # with pytest.raises(todo.NoDueDateError):
    assert todos.due_date("02/03/2011 Document +TodoTxt task format") == ""
...
def test_todo_highlight(todos):
    todos.parse_raw_entries(["25/10/2013 This is a +Very @cool test"])
    assert todos.todo_items[0].colored == ('plain', ['', ('creation_date', '25/10/2013'), ' This is a ', ('project', '+Very'), ' ', ('context', '@cool'), ' test'])
...
def test_todos_update(todos):
    todos.update([
        "(A) 24/12/1999 Thank Mom for the dinner @phone",
        "(B) Schedule Goodwill pickup +GarageSale @phone",
        "Unpack the guest bedroom +Unpacking due:20/10/2013",
        "19/10/2013 Post signs around the neighborhood +GarageSale",
        "x 01/10/2013 @GroceryStore Eskimo pies" ])
    assert [t.raw for t in todos] == [
        ...
        "x 01/10/2013 @GroceryStore Eskimo pies" ]
    todos.update([
        "x 25/12/1999 (A) 1999-12-24 Thank Mom for the dinner @phone"
        "x 10/11/1999 (B) Schedule Goodwill pickup +GarageSale @phone"
        "Unpack the guest bedroom +Unpacking due:20/10/2013",
        "19/10/2013 Post signs around the neighborhood +GarageSale",
        "x 01/10/2013 @GroceryStore Eskimo pies" ])
    assert [t.raw for t in todos] == [
...
def test_todos_complete(todos):
    ...
    todos[1].complete()
    assert [t.raw for t in todos] == [
        "x {} (A) 24/12/1999 Thank Mom for the dinner @phone".format(today),
    ...
    assert todos[3].creation_date == "19/10/2013"
...
def test_todo_incomplete(todos):
    ...
    assert todos[1].completed_date == "10/11/1999"
    todos[1].incomplete()
    assert todos[1].creation_date == ""
    assert todos[1].completed_date == ""

    assert todos[0].creation_date == "24/12/1999"
    assert todos[0].completed_date == "25/12/1999"
    todos[0].incomplete()
    assert todos[0].creation_date == "24/12/1999"
...
def test_todo_update(todos):
    t = todos[0]
    t.update("(A) 12/20/2000 Thank Mom for the dinner @phone @home +GiveThanks")
    assert t.raw            == "(A) 12/20/2000 Thank Mom for the dinner @phone @home +GiveThanks"
    assert t.priority       == "A"
    assert t.contexts       == ["@home", "@phone"]
    assert t.projects       == ["+GiveThanks"]
    assert t.creation_date  == "12/20/2000"
    assert t.due_date       == ""
    ...
    t = todos[2]
    t.update("x 25/10/2013 Unpack the guest bedroom +Unpacking due:2013-10-20")
    assert t.raw            == "x 25/10/2013 Unpack the guest bedroom +Unpacking due:2013-10-20"
    assert t.priority       == ""
    assert t.contexts       == []
    assert t.projects       == ["+Unpacking"]
    assert t.creation_date  == ""
    assert t.due_date       == "20/10/2013"
    assert t.completed_date == "25/10/2013"
    assert t.is_complete()  == True

    t = todos[3]
    assert t.raw           == "19/10/2013 Post signs around the neighborhood +GarageSale"
    assert t.contexts      == []
    assert t.projects      == ["+GarageSale"]
    assert t.creation_date == "19/10/2013"

    t = todos[4]
    assert t.raw            == "x 01/10/2013 @GroceryStore Eskimo pies"
    assert t.contexts       == ["@GroceryStore"]
    assert t.projects       == []
    assert t.completed_date == "01/10/2013"



def test_todo_add_creation_date(todos, today):
    todos[2].add_creation_date()
    assert todos[2].raw == "{} Unpack the guest bedroom +Unpacking due:20/10/2013".format(today)
    assert todos[2].creation_date == "{}".format(today)

    todos[3].add_creation_date()
    assert todos[3].raw == "19/10/2013 Post signs around the neighborhood +GarageSale"
    assert todos[3].creation_date == "19/10/2013".format(today)
...
def test_todos_search(todos):
    ...
    assert [t.raw for t in todos.search(".*")]             == [ "(A) 24/12/1999 .Thank* Mom for the .dinner* @phone" ]
    assert [t.search_matches for t in todos.search(".*")]  == [('.dinner*',)]
    assert [t.raw for t in todos.search("{b}")]            == [ "Unpack the guest {bedroom} +Unpacking due:20/10/2013" ]
